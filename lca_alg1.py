...
def best_shift(n0, n1, G, clustering, node2cid, trace_on=False):
    ...
    if trace_on:
        print("===========================")
        print("Running best_shift:  n0 = %s, n1 = %s" % (n0, n1))
...
def lca_alg1(curr_G, stop_at_two=False, trace_on=False):
    ...
    for e in pos_edges:
        ...
        if trace_on:
            print("is_merge_allowed", is_merge_allowed)

        if n0_cid == n1_cid:
            if trace_on:
                print("In the same cluster")
            score += wgt
        elif is_merge_allowed and \
            not ct.has_edges_between_them(G_prime, clustering[n0_cid],
                                          clustering[n1_cid]):
            if trace_on:
            ...
            if len(clustering[n0_cid]) == 1 or len(clustering[n1_cid]) == 1:
                ...
                if trace_on:
                    print("not checking moving nodes because "
                          "at least one cluster is length 1")
            else:
                sc_n0_to_n1, n0_to_move = best_shift(n0, n1, G_prime,
                                                     clustering, node2cid,
                                                     trace_on=trace_on)
                sc_n0_to_n1 += wgt
                if trace_on:
            ...
            if is_merge_allowed and \
               sc_merged >= max(sc_unmerged, sc_n0_to_n1, sc_n1_to_n0):
...
def test_best_shift(trace_on=False):
    ...
    if exp_delta != delta or set(exp_move) != set(to_move):
        print('Test 1 (larger to smaller): FAIL')
        print('    delta %a, to_move %a' % (delta, sorted(to_move)))
        print("    should be -12 and ['e', 'f', 'g']")
    else:
        print('Test 1 (larger to smaller): success')
    ...
    exp_move = ['c']
    if exp_delta != delta or set(exp_move) != set(to_move):
        print('Test 2 (smaller to larger): FAIL')
        print('delta %a, to_move %a' % (delta, sorted(to_move)))
        print("should be -26 and ['c']")
    else:
...
def test_run_lca_alg1(G, expected_clustering, msg,
    ...
    if score != expected_score:
        failed = True
        print("score %d, expected_score %d. FAILED" % (score, expected_score))
...
def test_no_final_merge(trace_on=False):
    ...
    print("=========================================\n"
          ...
          "=========================================")
    G = nx.Graph()
    G.add_weighted_edges_from([('a', 'b', 8), ('a', 'd', -1), ('a', 'e', 2),
                               ('b', 'c', 6), ('b', 'd', 3), ('b', 'e', 1),
                               ('c', 'f', 1), ('d', 'e', 4), ('e', 'f', 5)])
