...
def lca_alg1_constrained(curr_G, in_same=[], in_different=[], trace_on=False):
    ...
    for e in pos_edges:
        ...
        if (n0, n1) in in_same:
            if trace_on:
                print('Skipping (%a, %a) because already in graph' % (n0, n1))
            continue
...
def test_run_lca_alg2(G, best_clustering, exp_alt_clustering, msg, trace_on=False):
    ...
    if alt_score != exp_alt_score:
        failed = True
        print('score %d, expected_score %d. FAILED' % (alt_score, exp_alt_score))
